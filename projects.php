...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ( isset($_POST['task_name']) AND isset($_POST['id_task_project']) AND isset($_POST['id_user']) ) {
		...
		$statement = $connection->prepare('INSERT INTO tasks (id_user, id_project, task_status, task_name, task_description, task_color, deadline) VALUES
		(?, ?, ?, ?, ?, ?, ?)');
		$statement->execute(array($id_user, $id_project, $task_status, $task_name, $task_description, $task_color, $deadline));	
	...
	if(isset($_POST['edit_id_task'])) {	
		...
		$statement = $connection->prepare('UPDATE tasks SET task_name=?, task_description=?, task_color=?, deadline=? WHERE id_task=?');
		$statement->execute(array($edit_task_name, $edit_task_description, $edit_task_color, $deadline, $edit_id_task));	
